...
app.post('/api/grades', (req, res, next) => {
  ...
  if (!Number.isInteger(score) || score < 0 || score > 100) {
    throw new ClientError(400, 'score must be an integer between 1 - 100');
  }
  if (!name || !course) {
...
app.get('/api/grades/:gradeId', (req, res, next) => {
  ...
  db.query(sql, params)
    .then(result => {
      ...
      if (!grade) {
        throw new ClientError(404, `cannot find with gradeId ${gradeId}`);
      } else {
        res.json(grade);
      }
    })
...
app.delete('/api/grades/:gradeId', (req, res, next) => {
  ...
  db.query(sql, params)
    .then(result => {
      ...
      if (!deletedGrade) {
        throw new ClientError(404, `cannot find grade with id ${gradeId}`);
      } else {
        res.sendStatus(204);
      }
    })
